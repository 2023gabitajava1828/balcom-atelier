...
const Community = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      const {
        data: eventsData
      } = await supabase.from("events").select("*").eq("status", "upcoming").gte("event_date", new Date().toISOString()).order("event_date", {
        ascending: true
      });
      if (eventsData) {
      ...
      <main className="pt-20">
        <section className="py-20 bg-gradient-to-b from-background to-muted/30">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {!user && <Card className="max-w-2xl mx-auto p-8 text-center mb-12">
            ...
            {loading ? <div className="text-center py-12">
                ...
                {events.map(event => <EventCard key={event.id} event={event} onRSVP={handleRSVP} isRSVPd={rsvps.has(event.id)} />)}
              </div>}
          </div>
        </section>
      </main>
